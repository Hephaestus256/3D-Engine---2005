...
void get_screen_shot (void* buff);
    ...
    if (_keyb_stat[0x38].press) {
      ...
      if (_keyb_stat[0x1D].press) {
        ...
        if (_keyb_stat[16].press) {
          camera->pos.x += sin(camera->angle_xz) * cos(camera->angle_yz) * rec_inc / 12.0;
          camera->pos.y -= sin(camera->angle_yz) * rec_inc / 12.0;
          camera->pos.z += cos(camera->angle_xz) * cos(camera->angle_yz) * rec_inc / 12.0;
        }
...
//    type_point = true;
  if (_keyb_stat[0x31].press) {
    ...
    _bmp_pal[0] = load_mk_bmp ("xzplane.gfx");
    _bmp_pal[1] = load_mk_bmp ("rock.gfx");
    ...
    _bmp_pal[9] = load_mk_bmp ("conc_hl.gfx");
    curr_bmp = _first_mkb;
  }
   ...
   int mouse_x, int mouse_y, point_type* first_point, tri_group* first_group)
{
  ...
  if (_dos_creat(filename, 0x02, &handle))
    exit(5);
...
//  return atan(plane.m2 / cos(atan2(-1,plane.m1)));
  return plane.m2 * cos(atan(plane.m1));
}
...
void get_screen_shot (void* buff)
{
  ...
  if (_dos_open("800x600.bmp", 0x02, &handle))
    exit(5);

  _dos_read(handle, stuff, 54, NULL);
  _dos_write(handle, buff, _screen.x_res * _screen.y_res * 3, NULL);
