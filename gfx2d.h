...
inline void pxl_trans
  (int x, int y, int new_r, int new_g, int new_b, double intn, void* buff);
inline void read_x2_buff (int x, int y, int* r, int* g, int* b, void* buff);
    ...
    for (int y = 0; y < 8; y++)
      for (int x = 0; x < 8; x++)
        if (font[c + 0 + x * _screen.bytes_per_pxl + y * 24] ||
          ...
          pxl_trans (x + pos, y + row * 8,
            int(font[c + 0 + x * _screen.bytes_per_pxl + y * 24] * 1),
            int(font[c + 1 + x * _screen.bytes_per_pxl + y * 24] * 1),
            int(font[c + 2 + x * _screen.bytes_per_pxl + y * 24] * 1), .5, buff
        );
  }
}
  ...
  (int x, int y, int new_r, int new_g, int new_b, double intn, void* buff)
{
  ...
  double old_intn = 1.0 - intn;
  
  read_x2_buff (x, y, &old_r, &old_g, &old_b, buff);
  pxl (x, y, int(old_r * old_intn + new_r * intn),
             int(old_g * old_intn + new_g * intn),
             int(old_b * old_intn + new_b * intn), buff);

//  copy_scrn_buff (buff, 0);
//  getchar();
}
...
{
  char* p = (char*)buff;
  *r = *(p + (x + y * _screen.x_res) * 3 + 2) & 255;
  *g = *(p + (x + y * _screen.x_res) * 3 + 1) & 255;
  *b = *(p + (x + y * _screen.x_res) * 3 + 0) & 255;
}
